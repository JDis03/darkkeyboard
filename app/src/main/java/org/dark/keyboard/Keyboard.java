 ...
 * <p>
 * A keyboard consists of rows of keys. The layout file for a keyboard contains XML that looks like:
 * </p>
 * <pre>{@code
 * <Keyboard
...
public class Keyboard {
    ...
    /** Dead key placeholder character (dotted small circle) */
    public static final char DEAD_KEY_PLACEHOLDER = 0x25cc;
    ...
    // Edge flags for key positioning
    ...
    /** Current shift state of the keyboard */
    private int mShiftState = SHIFT_OFF;

    /** Key instances for modifier keys */
    @Nullable
    ...
    public static class Row {
        ...
        /** Whether this is an extension row */
        ...
        /**
         * Create an empty row with the given parent
         */
        public Row(@Nullable Keyboard parent) {
            this.parent = parent;
        }

        /**
         * Create a row from XML attributes
         *
         * @param res Resources for obtaining dimensions
         * @param parent The parent keyboard
         * @param parser XML parser containing row attributes
         */
        public Row(Resources res, Keyboard parent, XmlResourceParser parser) {
            ...
            try (TypedArray attrs = res.obtainAttributes(Xml.asAttributeSet(parser),
                    R.styleable.Keyboard_Row)) {
                mode = attrs.getResourceId(R.styleable.Keyboard_Row_keyboardMode, 0);
                extension = attrs.getBoolean(R.styleable.Keyboard_Row_extension, false);

                if (parent.mLayoutRows >= 5 || extension) {
                    // Apply optional scale factor to top (5th) row and/or extension row
                    final boolean isTop = (extension || parent.mRowCount - parent.mExtensionRowCount <= 0);
                    final float topScale = LatinIME.sKeyboardSettings.topRowScale;
                    // Apply scale factor to the top row(s), and redistribute the saved space
                    final float scale = isTop ?
                            topScale :
    ...
    public static class Key {
         ...
         */
        public Key(@Nullable Row parent) {
            keyboard = parent != null ? parent.parent : null;
            height = parent != null ? parent.defaultHeight : 0;
            width = parent != null ? Math.round(parent.defaultWidth) : 0;
            realWidth = parent != null ? parent.defaultWidth : 0f;
            gap = parent != null ? Math.round(parent.defaultHorizontalGap) : 0;
            realGap = parent != null ? parent.defaultHorizontalGap : 0f;
        }
        ...
        public Key(Resources res, Row parent, int x, int y, XmlResourceParser parser) {
            ...
            // First pass: get Keyboard-level attributes
            try (TypedArray a = res.obtainAttributes(Xml.asAttributeSet(parser),
                    R.styleable.Keyboard)) {
                realWidth = getDimensionOrFraction(a,
                        R.styleable.Keyboard_keyWidth,
                        keyboard != null ? keyboard.mDisplayWidth : 0, parent.defaultWidth);
                float realHeight = getDimensionOrFraction(a,
                        R.styleable.Keyboard_keyHeight,
                        keyboard != null ? keyboard.mDisplayHeight : 0, parent.defaultHeight);
                ...
                realGap = getDimensionOrFraction(a,
                        R.styleable.Keyboard_horizontalGap,
                        keyboard != null ? keyboard.mDisplayWidth : 0, parent.defaultHorizontalGap);
                if (parent != null && parent.parent != null) {
            ...
            // Second pass: get Key-specific attributes
            try (TypedArray a = res.obtainAttributes(Xml.asAttributeSet(parser),
                    R.styleable.Keyboard_Key)) {
                this.realX = this.x + realGap - (parent != null && parent.parent != null ? parent.parent.mHorizontalPad / 2 : 0);
                ...
                if (codesValue.type == TypedValue.TYPE_INT_DEC
                    ...
                    codes = parseCSV(codesValue.string != null ? codesValue.string.toString() : "");
                }
                ...
                if (codes == null && !TextUtils.isEmpty(label)) {
                    ...
                    if (codes != null && codes.length == 1) {
                        final Locale locale = LatinIME.sKeyboardSettings != null ? 
                                LatinIME.sKeyboardSettings.inputLocale : Locale.getDefault();
        ...
        public int[] getFromString(CharSequence str) {
            if (str.length() > 1) {
                if (str.charAt(0) == DEAD_KEY_PLACEHOLDER && str.length() >= 2) {
                    // Return first combining character for dead key sequences
                    return new int[] { str.charAt(1) };
                } else {
                    // Store full text for multi-character output
                    text = str;
                    return new int[] { 0 };
                }
            } else {
            ...
            while ((event = parser.next()) != XmlResourceParser.END_DOCUMENT) {
                if (event == XmlResourceParser.START_TAG) {
                    ...
                    if (inKey) {
                        ...
                        // Handle unexpected XML structure - extending keyboard bounds
                        Log.w(TAG, "Unexpected end element outside row context");
                    }
                }
            }
        } catch (Exception e) {
